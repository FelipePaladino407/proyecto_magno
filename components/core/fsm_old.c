...
//     ESP_LOGI("LCD_SIM", "Mostrar en LCD -> ID:%s | Nombre:%s | Stock:%lu", product->id, product->name,
...
//         ESP_LOGW(TAG, "Producto no registrado -> ID=%s | Nombre=%s", active_product.id, active_product.name);
...
//     ESP_LOGI(TAG, "Producto reconocido -> ID=%s | Nombre=%s | Stock actual=%lu", active_product.id,
//     active_product.name,
...
//     ESP_LOGI(TAG, "Stock actualizado -> ID=%s | Nombre=%s | Agregado=%lu | Stock=%lu", active_product.id,
//              active_product.name, (unsigned long)selected_quantity, (unsigned long)active_product.stock);
