...
static bool parse_scan_payload(const char *message, Product *out_product, time_t *out_timestamp, char *out_state,
                               size_t state_size) {
    if (message == NULL || out_product == NULL || out_timestamp == NULL || out_state == NULL || state_size == 0) {
        return false;
    }
...
static void publicar_catalogo_inicial(esp_mqtt_client_handle_t client) {
    ...
    while (i < catalog_size) {
        ...
        if (!telemetry_build_catalog_batch_payload(i, 10, timestamp, payload, sizeof(payload), &next_index)) {
        ...
        ESP_LOGI(TAG, "Catalogo inicial publicado [%u/%u]: %s", (unsigned)next_index, (unsigned)catalog_size, payload);
...
static void mqtt_hivemq_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    (void)handler_args;
    (void)base;
...
bool procesar_y_publicar_error(const char *mensaje_error) {
    ...
    safe_copy(error_product.name, mensaje_error != NULL ? mensaje_error : "Error desconocido",
              sizeof(error_product.name));
...
bool mqtt_handler_flush_pending(void) {
    ...
    while (pending_queue_count() > 0) {
        ...
        if (!publish_with_timestamp(pending_event.product, pending_event.state, pending_event.topic,
                                    pending_event.timestamp, true)) {
