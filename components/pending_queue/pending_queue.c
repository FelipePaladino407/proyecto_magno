...
static bool queue_is_valid(void) {
    return s_head >= 0 && s_head < PENDING_QUEUE_SIZE && s_tail >= 0 && s_tail < PENDING_QUEUE_SIZE && s_count >= 0 &&
           s_count <= PENDING_QUEUE_SIZE;
}
...
bool pending_queue_init(void) {
    ...
    ESP_LOGI(TAG, "Cola pendiente inicializada desde NVS: head=%d tail=%d count=%d", s_head, s_tail, s_count);
...
bool pending_queue_push(Product product, time_t timestamp, const char *state, const char *topic) {
    ...
    ESP_LOGI(
        ...
        slot->topic, slot->product.id, slot->product.name, (unsigned long)slot->product.stock,
        (long long)slot->timestamp, slot->state, s_count);
...
void pending_queue_print(void) {
    ...
    for (int i = 0; i < s_count; i++) {
        ...
        ESP_LOGI(TAG, "[%d] topic=%s | id=%s | nombre=%s | stock=%lu | timestamp=%lld | estado=%s", i, event->topic,
                 event->product.id, event->product.name, (unsigned long)event->product.stock,
                 (long long)event->timestamp, event->state);
    }
}
