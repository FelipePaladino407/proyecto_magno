...
#define RUN_TEST(name, seq, expected)                           \
    ...
    RUN_TEST("PRODUCT_FOUND -> PROMPT_ADD_PRODUCT",
             t5,
             STATE_PROMPT_ADD_PRODUCT);

    const EventType t6[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_NOT_FOUND};
    RUN_TEST("PRODUCT_NOT_FOUND -> ERROR_DISPLAY",
             t6,
             STATE_ERROR_DISPLAY);

    const EventType t7[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_FOUND, EV_BTN_CONFIRM};
    RUN_TEST("PROMPT_ADD_PRODUCT + CONFIRM -> QUANTITY_SELECTION",
             t7,
             STATE_QUANTITY_SELECTION);

    const EventType t8[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_FOUND, EV_BTN_CANCEL};
    RUN_TEST("PROMPT_ADD_PRODUCT + CANCEL -> IDLE",
             t8,
             STATE_IDLE);

    const EventType t9[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_FOUND, EV_BTN_CONFIRM, EV_BTN_UP};
    RUN_TEST("QUANTITY_SELECTION + UP -> QUANTITY_SELECTION",
             t9,
             STATE_QUANTITY_SELECTION);

    const EventType t10[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_FOUND, EV_BTN_CONFIRM, EV_BTN_DOWN};
    RUN_TEST("QUANTITY_SELECTION + DOWN -> QUANTITY_SELECTION",
             t10,
             STATE_QUANTITY_SELECTION);

    const EventType t11[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_FOUND, EV_BTN_CONFIRM, EV_BTN_CONFIRM};
    RUN_TEST("QUANTITY_SELECTION + CONFIRM -> STOCK_UPDATING",
             t11,
             STATE_STOCK_UPDATING);

    const EventType t12[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_FOUND, EV_BTN_CONFIRM, EV_BTN_CONFIRM, EV_STOCK_UPDATED};
    RUN_TEST("STOCK_UPDATING + STOCK_UPDATED -> DISPLAY_PRODUCT_INFO",
             t12,
             STATE_DISPLAY_PRODUCT_INFO);

    const EventType t13[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_FOUND, EV_BTN_CONFIRM, EV_BTN_CONFIRM, EV_STOCK_UPDATED, EV_TIMEOUT};
    RUN_TEST("DISPLAY_PRODUCT_INFO + TIMEOUT -> MQTT_PUBLISHING",
             t13,
             STATE_MQTT_PUBLISHING);

    const EventType t14[] = {EV_QR_CAPTURED, EV_SCAN_SUCCESS, EV_PRODUCT_FOUND, EV_BTN_CONFIRM, EV_BTN_CONFIRM, EV_STOCK_UPDATED, EV_TIMEOUT, EV_MQTT_PUBLISH_SUCCESS};
    RUN_TEST("MQTT_PUBLISHING + SUCCESS -> IDLE",
             t14,
             STATE_IDLE);
