...
static esp_err_t load_credentials(char *ssid, size_t ssid_size, char *password, size_t password_size) {
    ...
    err = nvs_storage_get_str("ssid", ssid, ssid_size);

    if (err != ESP_OK)
        return err;

    err = nvs_storage_get_str("password", password, password_size);
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if (event_base == WIFI_EVENT) {
        ...
        case WIFI_EVENT_STA_START:
            rgb_led_set_color(255, 255, 255); // Blanco
    ...
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        char ip_string[16];
        snprintf(ip_string, sizeof(ip_string), IPSTR, IP2STR(&ev->ip_info.ip));
...
// la contraseña.
static void fill_sta_config(wifi_config_t *config, const char *ssid, const char *password) {
...
esp_err_t wifi_manager_init(void) {
    ...
    ESP_RETURN_ON_ERROR(esp_wifi_init(&init_cfg), TAG, "No se pudo inicializar el driver Wi-Fi");

    ESP_RETURN_ON_ERROR(
        esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, wifi_event_handler, NULL, NULL), TAG,
        "No se pudo registrar el handler Wi-Fi");

    ESP_RETURN_ON_ERROR(
        esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, wifi_event_handler, NULL, NULL), TAG,
        "No se pudo registrar el handler IP");

    ESP_RETURN_ON_ERROR(esp_wifi_set_storage(WIFI_STORAGE_RAM), TAG, "No se pudo configurar el almacenamiento Wi-Fi");
    ...
    snprintf((char *)ap_config.ap.ssid, sizeof(ap_config.ap.ssid), "ESP32-%02X%02X%02X", mac[3], mac[4], mac[5]);
    ap_config.ap.ssid_len = strlen((char *)ap_config.ap.ssid);

    if (strlen(WIFI_MANAGER_AP_PASS) == 0)
        ap_config.ap.authmode = WIFI_AUTH_OPEN;

    ESP_RETURN_ON_ERROR(esp_wifi_set_mode(WIFI_MODE_APSTA), TAG, "No se pudo seleccionar el modo AP+STA");
    ESP_RETURN_ON_ERROR(esp_wifi_set_config(WIFI_IF_AP, &ap_config), TAG, "No se pudo configurar el AP");
...
esp_err_t wifi_manager_set_credentials(const char *ssid, const char *password) {
    ...
    ESP_RETURN_ON_ERROR(save_credentials(ssid, password), TAG, "No se pudieron guardar las credenciales");
    ...
    ESP_RETURN_ON_ERROR(esp_wifi_set_config(WIFI_IF_STA, &sta_config), TAG, "No se pudo aplicar la configuracion STA");
